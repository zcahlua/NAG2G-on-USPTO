...
echo "time" $time
[ -z "${task_name:-}" ] && task_name='NAG2G_unimolplus_uspto_50k'
...
if echo "${gpu}" | grep -q "V100"; then
    ...
    echo "Using fp32 fallback for GPU/accelerator: $gpu"
    gpu_type=""
    [ -z "${num_workers:-}" ] && num_workers=12
fi
echo ${gpu_type}
